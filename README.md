...
- 🔍 **Семантический поиск** - умный поиск по смыслу, а не только по ключевым словам
...
- **Embeddings**: [nomic-embed-text](https://ollama.com/library/nomic-embed-text) для поиска
- **Vector Store**: [Qdrant](https://qdrant.tech) (local embedded mode)
...
- Загрузит модели Whisper, Qwen и Embeddings
- Соберет whisper.cpp с поддержкой Core ML
- Создаст базу данных Qdrant и проиндексирует заметки
...
| `/search <запрос>` | Поиск по содержимому (семантический) |
| `/health` | Проверка состояния системы |
| `/cleanup` | Очистка временных файлов |

### 🔍 Семантический поиск

Бот понимает смысл вашего запроса. 
Например, если вы ищете "что купить на ужин", он найдет заметку "список продуктов", даже если там нет слова "ужин".

Нажмите кнопку **🔍 Поиск** или отправьте текст боту для быстрого поиска.

**Особенности поиска:**
- **Умная выдача:** Бот анализирует ответ нейросети. Если информация в заметках не найдена, он ответит честно и не будет прикреплять источники или лишние файлы.
- **RAG:** Бот использует контекст нескольких наиболее подходящих заметок для генерации точного ответа.
...
```bash
# Проверьте сборку или установите через Homebrew
brew install whisper-cli

# Обновите путь в .env если нужно
...
Если вы удалили файлы или добавили заметки вручную, запустите скрипт для полной очистки и обновления базы поиска:

```bash
./scripts/index.sh
```

> ⚠️ **Важно:** Перед запуском индексации остановите бота (`Ctrl+C`), так как база Qdrant в локальном режиме позволяет работать только одному процессу одновременно.
