...
### 1.5 Semantic Search & RAG
**Назначение:** Поиск заметок по смыслу и формирование ответов по контексту
...
3. При поиске (через голосовые команды "Найди", "Где" и т.д.) запрос также превращается в вектор.
4. Выполняется поиск топ-2 наиболее релевантных заметок.
5. **RAG Logic**: Нейросеть (Qwen) формирует краткий ответ на вопрос, основываясь только на предоставленном контексте найденных заметок.
6. **Cleanup Logic**: Скрипт переиндексации (`reindex.py`) полностью очищает коллекцию перед запуском, чтобы исключить удаленные файлы из поиска.

> ⚠️ **Важно:** Qdrant в Embedded режиме блокирует файлы базы. Бот и скрипт переиндексации не могут работать одновременно.
