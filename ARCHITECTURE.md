...
└─────────────┘     └──────────────┘     └─────────────┘     └──────────────┘     └────────────┘
                           │                    │                     │                    │
                           ▼                    ▼                     ▼                    ▼
                    audio.ogg            transcription.txt      structured.md        categories/
                                                                                      ├─ идеи/
                                                                                      ├─ жизнь/
                                                                                      ├─ работа/
                                                                                      └─ инбокс/
...
**Выходные данные:**
- JSON с полями:
  - `category` — категория (идеи/жизнь/работа/инбокс)
  - `title` — заголовок заметки
  - `content` — отформатированный markdown
...
```python
SYSTEM_PROMPT = """Ты — ассистент для обработки голосовых заметок. Твоя задача:
...
Категории:
- "идеи" — творческие идеи, проекты, концепции
- "жизнь" — личные дела, покупки, планы
- "работа" — рабочие задачи, встречи, проекты
...
Верни JSON с category, title и content."""
```
...
async def stats(update, context):
    """
    Команда: /stats
    Показывает статистику по заметкам
    """
    notes_dir = Path(os.getenv('NOTES_DIR'))
    ...
    message = (
        ...
        f"💡 Идеи: {stats['идеи']}\n"
        f"🌱 Жизнь: {stats['жизнь']}\n"
        f"💼 Работа: {stats['работа']}\n"
        f"📥 Инбокс: {stats['инбокс']}\n\n"
        f"**Всего: {total}**"
    )
