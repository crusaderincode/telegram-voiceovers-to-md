...
class FileManager:
    ...
    def move_note(self, filename: str, from_category: str, to_category: str) -> bool:
        """
        Перемещение заметки между категориями
        
        Args:
            filename: Имя файла
            from_category: Текущая категория
            to_category: Новая категория
            
        Returns:
            bool: Успешно ли перемещено
        """
        if to_category not in Settings.CATEGORIES:
            logger.error(f"Invalid target category: {to_category}")
            return False
        
        from_path = self.notes_dir / from_category / filename
        to_path = self.notes_dir / to_category / filename
        
        if not from_path.exists():
            logger.error(f"Source file not found: {from_path}")
            return False
        
        if to_path.exists():
            logger.error(f"Target file already exists: {to_path}")
            return False
        
        try:
            # Обновляем категорию в метаданных
            content = from_path.read_text(encoding='utf-8')
            content = re.sub(
                r'category:\s*\w+',
                f'category: {to_category}',
                content
            )
            ...
            logger.info(f"Moved {filename}: {from_category} → {to_category}")
            return True
